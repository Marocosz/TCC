...
informacionais, moldando a chamada ``arquitetura da escolha'' de milhões de
...
consolidando modelos de negócios associados à chamada Economia da Atenção. Nesse
...
\begin{itemize}
  ...
  \item Coletar e processar, via API do Spotify, os metadados técnicos e
  socioeconômicos das recomendações geradas (os seis \emph{Daily Mixes} de cada
  perfil), estruturando um \emph{corpus} de dados comparável entre os diferentes
  perfis.
...
Essa lógica articula-se ao modelo de negócio da \textbf{economia da atenção}, no
qual o sucesso da plataforma depende da maximização da retenção do usuário. Daí
...
($G$) auditou a concentração de \emph{plays} entre os artistas. Um Gini próximo
...
\textbf{A) Popularidade Média e Zona de Segurança.} Calculada a partir do índice
\texttt{track\_popularity} (0 a 100) fornecido pela API do Spotify, que pondera o
número total de reproduções e a recência dessas execuções. Uma média alta
($>$70) indica um perfil na ``Zona de Segurança Algorítmica''; uma média baixa
($<$20) indica consumo na ``Zona de Risco'', onde o algoritmo possui menos dados
históricos de engajamento.

\textbf{B) Consistência Interna (Desvio Padrão da Popularidade).} Métrica
estatística ($\sigma$) que mede a variação dos índices de popularidade dentro da
mesma \emph{playlist}. Um desvio padrão baixo (ex.: $\pm 4{,}0$) indica curadoria
homogênea; um desvio alto indica comportamento errático, misturando
\emph{superstars} com artistas desconhecidos.

\textbf{C) Recência Temporal e Viés de Imediatismo.} Análise da distribuição dos
anos de lançamento dos álbuns (\texttt{release\_date}). Perfis com $>$80\% das
...
\textbf{D) Alcance Médio (Capital Social).} Média do número de seguidores
(\texttt{followers.total}) dos artistas presentes na biblioteca. Serve como
\emph{proxy} para o capital econômico e social, distinguindo ``Artistas de
Estádio'' (milhões de seguidores) de ``Artistas de Nicho'' (milhares).
...
às estimativas pontuais, implementado em
\texttt{src/analysis/build\_significance.py} (semente aleatória fixa para
reprodutibilidade). As ferramentas foram escolhidas conforme o nível de agregação
...
completos estão em \texttt{reports/comparison/} (\texttt{significancia.csv},
\texttt{significancia\_mannwhitney.csv}, \texttt{rarefacao.csv},
\texttt{jaccard\_significancia.csv}).

\subsection{Adaptação Metodológica: Migração para Fontes Externas (Last.fm + MusicBrainz)}
\label{sec:adaptacao}

Durante a execução da pesquisa, observou-se que a Spotify Web API sofreu
\textbf{três ondas progressivas de restrição} ao longo de 2024 e 2026, afetando
diretamente os campos centrais utilizados na análise quantitativa do estudo. Este
fato, longe de constituir apenas uma limitação operacional, revelou-se um achado
original com valor científico próprio, evidenciando empiricamente o argumento
central desta pesquisa sobre a opacidade e a governança algorítmica unilateral
exercida por plataformas de \emph{streaming}.

\textbf{Cronologia das restrições documentadas:}

\begin{enumerate}
  \item \textbf{Onda 1 (27/11/2024):} a Spotify removeu, para aplicações em modo
  ...
  \item \textbf{Onda 3 (Fevereiro/2026, sem \emph{changelog} público):}
  verificação empírica conduzida em 28 de abril de 2026 evidenciou a remoção
...
Cauda Longa (\emph{Long Tail}) foram recalibrados via \textbf{percentis sobre um
...
do experimento. Beatriz representa o comportamento padrão ``ideal'' para modelos
de negócio baseados em economia da atenção: consumo rápido, alta retenção e foco
...
distribuição de Pareto. Os limiares de classificação foram \textbf{calibrados via
percentis sobre um \emph{pool} único} que combina os oito conjuntos do estudo
...
\emph{listeners} no Last.fm. Essa \textbf{régua única} --- idêntica para
...
Popularidade $\times$ Seguidores).} Este gráfico espacializa a Teoria da Cauda
Longa (\emph{The Long Tail}). O Quadrante Superior Direito (\emph{Head}) é
...
Durante o período de execução desta pesquisa (2025--2026), a Spotify Web API
sofreu \textbf{três ondas progressivas e parcialmente não-anunciadas de
restrição}, conforme detalhado na Seção~\ref{sec:adaptacao}: a Onda 1 (Nov/2024)
removeu o acesso a \emph{playlists} algorítmicas para \emph{apps} em modo
\emph{development}; a Onda 2 (Fev/2026) exigiu \emph{Premium} para o titular do
...
colaborador; e a Onda 3 (Fev/2026, sem \emph{changelog} público) removeu
...
e a governança algorítmica} das plataformas de \emph{streaming} --- o próprio
instrumental técnico necessário à auditoria foi sendo obstruído \emph{durante} a
execução do experimento. Esta observação configura-se como \textbf{meta-evidência}
...
computada em \texttt{build\_significance.py}. Para as medianas de audiência,
...
Figura~\ref{fig:kde-era} evidencia que, embora a distribuição temporal permaneça
estável, a composição interna da bolha mudou drasticamente.
...
declarado de ``obras completas''. Adicionalmente, fragmenta o consumo vertical
...
\label{tab:jaccard}
\footnotesize
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3cm} c c c X}
\toprule
...
Ricardo & Estagnamento de descoberta (\emph{loop} temporal) & Pulverização ($+600\%$) --- oposto da hipótese & Extrema (diversidade) & Refutada e substituída \\
\bottomrule
\end{tabularx}
\fontefig{Elaborado pelo autor (2026).}
\end{table}
...
profundidade do que respeitá-la.
...
famílias de métricas independentes) e o uso de duas fontes externas validadas
